...
partial def inferBindSourceType
  ...
  -- corresponding structured panic check. They match the surface of Solidity's `a + b` / `a - b` / `a * b`
...
def translateSafeRequireBind
  ...
  -- Solidity-0.8 default-revert arithmetic (verity#1752): `addPanic`,
  -- `subPanic`, `mulPanic`, and `divPanic` lower to a direct failure check
  -- followed by a typed `Stmt.panic`. This preserves Solidity 0.8's
  -- canonical `Panic(0x11)` (overflow / underflow) and `Panic(0x12)`
  -- (division by zero) payloads without routing through `Error(string)`.
  | `(term| addPanic $a:term $b:term) =>
      let aExpr ← translateOperand a
      let bExpr ← translateOperand b
      let valueExpr : Term ← `(Compiler.CompilationModel.Expr.add $aExpr $bExpr)
